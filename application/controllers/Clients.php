...
/*get client list or add new client */
    function choose_user(){
     $this->access_only_allowed_members();
        ...
        $view_data['groups_dropdown'] = json_encode($this->_get_groups_dropdown_select2_data(true));
      $this->load->view('projects/choose_user', $view_data);
    }
    ...
    function modal_form() {
        ...
        $view_data['model_info'] = $this->db->query("select clients.id, clients.*, users.first_name, users.last_name, users.email, users.user_type from users left join clients on clients.id=users.client_id where client_id=" . $client_id)->result_object()[0];
    ...
    function save() {
        ...
        validate_submitted_data(array(
            "id" => "numeric", "first_name" => "required", "last_name" => "required"
        ));
