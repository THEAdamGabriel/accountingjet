...
class Projects extends MY_Controller {
    ...
    function services_errors($key){
        $this->load->helper(array('form', 'url'));
        $this->load->library('form_validation');
       $errors = array();
       switch($key){
            case('Bookkeeping'):
                if($_GET['services']['Bookkeeping'] ==  1){
                 
                        if(!is_numeric($_GET['num_accounts'])){
                           $errors['num_accounts'] = array('msg' => 'Please tell us how many bank accounts you have', 'field' => 'num_accounts');
                        }
                        if(!is_numeric($_GET['expenses'])){
                           $errors['expenses'] = array('msg' => 'Please tell us what your total monthly expenses are', 'field' => 'expenses');
                        }
                    
                }
            break;
            case('Taxes'):
               if($_GET['services']['Taxes'] ==  1){
                   if(!is_numeric($_GET['bus_annual_income']) | empty($_GET['bus_annual_income'])){
                           $errors['bus_annual_income'] = array('msg' => 'What is your average annual business income', 'field' => 'bus_annual_income');
                        }
                 }    
            break;
            case('Payroll'):
                if($_GET['services']['Payroll'] ==  1){
                  if(!is_numeric($_GET['num_employees']) ){
                           $errors['num_employees'] = array('msg' => 'Please tell us how many employees you have', 'field' => 'num_employees');
                        }
    ...
    public function deduction_types(){
      ...
      $data1 = $this->db->query("select distinct type from deduction_types")->result_array();
      $data = array();
    ...
    function delete_client_files(){
      ...
      $this->db->query("delete from client_project_files where id=" . $_GET['id'] . " and client_id=". $_GET['client_id']);
      $files = $this->db->query("select filename, filetype, id from client_project_files where client_id=" . $_GET['client_id'] . " and type='" . $_GET['type'] . "'")->result_object();
      echo json_encode($files);
    }
    function delete_record(){
      $this->db->query("delete from dependant_and_spouse where id=" . $_GET['id'] . " and client_id=" . $_GET['client_id']);
    ...
    function uploader(){
        header('content-type: application/json');
        $allowed = array('pdf', 'xlsx', 'xls');
        $ext = explode('/', $_FILES['file']['type'])[1];
        if(!in_array($ext, $allowed)){
         echo json_encode(array('error' => 'Must be a PDF or Excel Document'));
         exit;
        
        }
        $content = file_get_contents($_FILES['file']['tmp_name']);
        
        $this->db->query("insert into client_project_files values(null, '" . $_FILES['file']['name'] . "', '" . $_FILES['file']['type'] . "', '" . addslashes($content) . "', '" . $_POST['type'] . "', '" . $_POST['client_id'] . "');");
    ...
    public function getPrice($key){
        ...
        switch($key){
            case('Bookkeeping'):
                if($_GET['services']['Bookkeeping'] ==  1){
                        $sprice = $this->db->query("select * from " . strtolower($key) ."_pricing where min_accounts<=" . $_GET['num_accounts'] . " order by min_accounts desc limit 1")->row();
                        $eprice = $this->db->query("select * from " . strtolower($key) ."_expenses_pricing where max_expenses<=" . $_GET['expenses'] . " order by max_expenses desc limit 1")->row();
                        if($this->db->query("select * from projects where client_id=" . $_GET['client_id'] . " and title='Bookkeeping'")->num_rows()==0){
                          $insert = $this->db->insert_string('projects', array('title' => 'Bookkeeping', 'client_id' => $_GET['client_id'], 'price' => $sprice->price +  $eprice->price, 'created_date' => 'NOW()'));
                          $this->db->query($insert);
                        }
                    
                      $serviced = $this->db->query("select * from projects where client_id=" . $_GET['client_id'] . " and title='Bookkeeping'")->row();  
                      $this->db->query('update projects set num_accounts= '. $_GET['num_accounts'] . ', price=' . ($sprice->price +  $eprice->price) . ', bookkeeping=1, expenses=' . $_GET['expenses'] . ', accounting_system="' . $_GET['accounting_system'] . '" where id=' . $serviced->id );
                }else {
                  $sprice = (object) array('price' => 0);
                  $eprice = (object) array('price' => 0);
                  $this->db->query("delete from projects where client_id=" . $_GET['client_id'] . " and title='Bookkeeping'");
                }
            break;
            case('Taxes'):
               if($_GET['services']['Taxes'] ==  1){
            //   echo "select * from " . strtolower($key) ."_pricing where max_income<=" . $_GET['bus_annual_income'] . " order by max_income desc limit 1";
            
                 switch($_GET['taxes_b']){
                        case('bus'):
                                $eprice = $this->db->query("select * from " . strtolower($key) ."_pricing where max_income<=" . $_GET['bus_annual_income'] . " order by max_income desc limit 1")->row();
                                $sprice = (object) array('price' => 499);
                        break;
                        case('per'):
                                $eprice = (object) array('price' => 0);
                                $sprice = (object) array('price' => 99);
                        break;
                        case('both'):
                                $eprice = $this->db->query("select * from " . strtolower($key) ."_pricing where max_income<=" . $_GET['bus_annual_income'] . " order by max_income desc limit 1")->row();
                                $sprice = (object) array('price' => 598);
                        break;
                 }
                  if($this->db->query("select * from projects where client_id=" . $_GET['client_id'] . " and title='Taxes'")->num_rows()==0){
                          $insert = $this->db->insert_string('projects', array('title' => 'Taxes', 'client_id' => $_GET['client_id'], 'client_id' => $_GET['client_id'], 'price' => $sprice->price +  $eprice->price, 'created_date' => 'NOW()'));
                          $this->db->query($insert);
                  }
                      $serviced = $this->db->query("select * from projects where client_id=" . $_GET['client_id'] . " and title='Taxes'")->row();  
                      if(isset($_GET['jointly'])){
                        $joint = $_GET['jointly'];
                      }else{
                        $joint='';
                        }
                      $this->db->query('update projects set bus_annual_income= '. $_GET['bus_annual_income'] . ', price=' . ($sprice->price +  $eprice->price) . ',  taxes="' .  $_GET['services']['Taxes'] . '", taxes_b="' . $_GET['taxes_b'] . '", corp_type="' . $_GET['corp_type'] . '", jointly="' . $joint . '" where id=' . $serviced->id );
                      
                      
                      $this->db->query("delete from dependant_and_spouse where client_id=" . $_GET['client_id'] . " and relationship='spouse'");
                      if(isset($_GET['spouse_name'])){
                            $sql = $this->db->insert_string('dependant_and_spouse', array('dob' => $_GET['spouse_dob'], 'ssn' => $_GET['spouse_ssn'], 'name' => $_GET['spouse_name'], 'client_id' => $_GET['client_id'], 'relationship' => 'spouse')); 
                       
                        
                        $this->db->query($sql);
                      }
                      $this->db->query("delete from dependant_and_spouse where client_id=" . $_GET['client_id'] . " and relationship='dependant'");
                      if(isset($_GET['dependant'])){
                        foreach($_GET['dependant']['name'] as $k => $dependant){
                            $sql = $this->db->insert_string('dependant_and_spouse', array('dob' => $_GET['dependant']['dob'][$k], 'ssn' => $_GET['dependant']['ssn'][$k], 'name' => $_GET['dependant']['name'][$k], 'client_id' => $_GET['client_id'], 'relationship' => 'dependant')); 
                            $this->db->query($sql);
                         
                        }
                      }
                      
                      $this->db->query("delete from dependant_and_spouse where client_id=" . $_GET['client_id'] . " and relationship='property'");
                      if(isset($_GET['property'])){
                        foreach($_GET['property']['name'] as $k => $property){
                          if(!empty($_GET['property']['name'][$k])){
                            $sql = $this->db->insert_string('dependant_and_spouse', array('name' => $_GET['property']['name'][$k], 'client_id' => $_GET['client_id'], 'relationship' => 'property')); 
                            $this->db->query($sql);
                            $sprice->price = $sprice->price + 99;
                          }  
                        }
                      }
                      $this->db->query("delete from dependant_and_spouse where client_id=" . $_GET['client_id'] . " and relationship='business'");
                      if(isset($_GET['business'])){
                        foreach($_GET['business']['name'] as $k => $property){
                         if(!empty($_GET['property']['name'][$k])){
                            $sql = $this->db->insert_string('dependant_and_spouse', array('name' => $_GET['property']['name'][$k], 'client_id' => $_GET['client_id'], 'relationship' => 'business')); 
                            $this->db->query($sql);
                            $sprice->price = $sprice->price + 99;
                          }  
                        }
                     ...
                     $eprice = (object) array('price' => 0);
                     $sprice = (object) array('price' => 499);
                     $this->db->query("delete from projects where client_id=" . $_GET['client_id'] . " and title='Taxes'");
                  }
            break;
            case('Payroll'):
                if($_GET['services']['Payroll'] ==  1){
                    $sprice = $this->db->query("select * from " . strtolower($key) ."_pricing where id=" . $_GET['payment_interval'] . " order by id desc limit 1")->row();
            
                    $eprice = $this->db->query("select * from " . strtolower($key) ."_expenses_pricing where num_employees<=" . $_GET['num_employees'] . " order by num_employees desc limit 1")->row();
                 
                    if($this->db->query("select * from projects where client_id=" . $_GET['client_id'] . " and title='Payroll'")->num_rows()==0){
                          $insert = $this->db->insert_string('projects', array('title' => 'Payroll', 'client_id' => $_GET['client_id'], 'client_id' => $_GET['client_id'], 'price' => ($sprice->price +  $eprice->price), 'created_date' => 'NOW()'));
                          $this->db->query($insert);
                    }
                        $serviced = $this->db->query("select * from projects where client_id=" . $_GET['client_id'] . " and title='Payroll'")->row();  
                      $this->db->query('update projects set num_employees= '. $_GET['num_employees'] . ', price= ' . ($sprice->price +  $eprice->price) . ', payroll=1, pay_types="' . $_GET['pay_types'] . '", payment_interval="' . $_GET['payment_interval'] . '", retirement="' . $_GET['retirement'] . '" where id=' . $serviced->id );
                    
                }else {
                  $sprice = (object) array('price' => 0);
                  $eprice = (object) array('price' => 0);
                  $this->db->query("delete from projects where client_id=" . $_GET['client_id'] . " and title='Payroll'");
                }

            break;
       }
     
        return array('sprice' => $sprice, 'eprice' => $eprice, 'total' => $sprice->price +  $eprice->price );
    ...
    public function prices(){
      ...
      foreach($_GET['services'] as $key => $value){
         ...
         $errors[$key] = $this->services_errors($key);
         //print_r($errors[$key]);
      ...
      if(count($errors)>0){
        ...
        $new_price = $this->getPrice($key);
        echo json_encode(array('success' => true,  'prices' => $new_price));
    ...
    function esign(){
      require(APPPATH . 'third_party/docusign.php');
      $esign = new UnitTests();
      $user = $this->db->query("select * from users where client_id=" . $_GET['client_id'])->row();
      define('DOCUEMAIL', $user->email);
      define('DOCUNAME', $user->first_name . ' ' . $user->last_name);
      $documents = $this->db->query("select distinct filename, content from client_project_files where client_id=" . $_GET['client_id'])->result_object();
      
      $esign->signatureRequestOnDocument($esign->testLogin(), $documents);
      
      return $this->load->view('projects/check_email');
    }
    function subscription(){
      require(APPPATH . '/third_party/Stripe/init.php');
      
      $view_data['model_info'] = $this->db->query("select clients.id, clients.*, users.id as user_id, users.first_name, users.last_name,  users.middle_name, users.email, users.user_type from users left join clients on clients.id=users.client_id where client_id=" . $_GET['client_id'])->result_object()[0];
      $view_data['model_info']->projects = $this->db->query("select count(id) as total, sum(price) as price from projects where client_id=" . $view_data['model_info']->id)->row();
    
      if(isset($_POST['ccnumber'])){
        \Stripe\Stripe::setApiKey('sk_test_RTcEowqi00YxvjVentAQVrsx00CQVQvIE9');
        $myCard = array('number' => $this->input->post('ccnumber'), 'exp_month' => $this->input->post('exp_month'), 'exp_year' => $this->input->post('exp_year'));
        $charge = \Stripe\Charge::create(array('card' => $myCard, 'amount' => 2000, 'currency' => 'usd'));
        try {
        
        } catch (Exception $charge) {
          $this->load->view('creditcardform', $view_data);
        
        }
      exit;
      redirect('projects/esign?client_id=' . $client_id);
      }else{
    ...
    function identification(){
      ...
      $subject = 'Verify Identificaton for ';
      $to = 'edward.goodnow@gmail.com';
      
      $message = 'Please verify the following ID card';
      $message .= '<img src="data:image/png;base64,' . base64_encode(file_get_contents($_FILES['inp']['tmp_name'])) . '" />';
      
      $view_data['model_info'] = $this->db->query("select clients.id, clients.*, users.id as user_id, users.first_name, users.last_name,  users.middle_name, users.email, users.user_type from users left join clients on clients.id=users.client_id where client_id=" . $client_id)->result_object()[0];
      
      $subject .= $view_data['model_info']->first_name . ' ' . $view_data['model_info']->last_name;
      ...
      $result = $this->email
       ...
       redirect('projects/subscription?client_id=' . $client_id);
    ...
    function blockscore(){
        require(APPPATH . 'third_party/blockscore.php');
            ...
            if ($this->form_validation->run() == FALSE)
                {
                  ...
                  $sql = $this->db->update_string('users', array('first_name' => $_POST['first_name'], 'last_name' => $_POST['last_name'], 'middle_name' => $_POST['middle_name']), array('client_id' => $_POST['id']));
                  $this->db->query($sql);
                  
                  
                  $sql =$this->db->update_string('clients', array('address' => $_POST['address'], 'city' => $_POST['city'], 'state' => $_POST['state'], 'dob' => $_POST['dob'],  'zip' => $_POST['zip'], 'country' => 'US', 'phone' => $_POST['phone'], 'ssn' => $_POST['ssn']), array('id' => $_POST['id']));
                  $this->db->query($sql);
                }
        }else{
        ...
        $user = $this->db->query("select clients.id, clients.*, users.first_name, users.last_name, users.middle_name, users.email, users.user_type from users left join clients on clients.id=users.client_id where client_id=" . $client_id)->row();
        
        if(empty($user->first_name) + empty($user->last_name) | empty($user->ssn) | empty($user->dob) | empty($user->address) | empty($user->city) | empty($user->state) | empty($user->zip) | empty($user->country)){
                ...
                $view_data['model_info'] = $this->db->query("select clients.id, clients.*, users.first_name, users.last_name, users.middle_name, users.email, users.user_type from users left join clients on clients.id=users.client_id where client_id=" . $client_id)->result_object()[0];
                ...
                return $this->load->view('projects/client_form_fields', $view_data);
          exit;
        }
        ...
        $person = \BlockScore\Person::create(array(
            'name_first' => $view_data['model_info']->first_name,
            'name_last' => $view_data['model_info']->last_name,
            'name_middle' => $view_data['model_info']->middle_name,
            'phone_number' => $view_data['model_info']->phone,
            'birth_month' => explode('-', $view_data['model_info']->dob)[1],
            'birth_day' => explode('-', $view_data['model_info']->dob)[2],
            'birth_year' => explode('-', $view_data['model_info']->dob)[0],
            'document_type' => 'ssn',
            'document_value' => $view_data['model_info']->ssn,
            'address_street1' => $view_data['model_info']->address,
            'address_city' => $view_data['model_info']->city,
            'address_subdivision' => $view_data['model_info']->state,
            'address_postal_code' => $view_data['model_info']->zip,
            'address_country_code' => $view_data['model_info']->country,
            
        ));
       
        if($person->details->identification == 'no_match'){
          if($_SESSION['user_id'] == $view_data['model_info']->user_id){
            return $this->load->view('clients/identification', $view_data);
          }else{
            $this->db->query("update clients set id_confirmed=1 where id=" . $client_id);
            redirect('projects/esign?client_id=' . $client_id );
            exit;
          }
        }else{
    ...
    function modal_form() {
        ...
        $sql = "select * from projects where client_id=$client_id and title='Bookkeeping'";
        if($this->db->query($sql)->num_rows()==0){
          $view_data['model_info']->Bookkeeping = (object) array('num_accounts' => 1, 'accounting_system' => '', 'expenses' => 0, 'bookkeeping' => '' );
        }else{
            $view_data['model_info']->Bookkeeping = $this->db->query($sql)->row();
        }
        $sql = "select * from projects where client_id=$client_id and title='Payroll'";
        if($this->db->query($sql)->num_rows()==0){
          $view_data['model_info']->Payroll = (object) array('num_employees' => 1, 'payment_interval' => '0', 'retirement' => 0, 'pay_types' => 1, 'price' => 0, 'payroll' => '' );
        }else{
            $view_data['model_info']->Payroll = $this->db->query($sql)->row();
        }
        $sql = "select * from projects where client_id=$client_id and title='Taxes'";
        if($this->db->query($sql)->num_rows()==0){
          $view_data['model_info']->Taxes = (object) array('num_accounts' => 1, 'accounting_system' => '', 'expenses' => 0, 'taxes' => '', 'price' => 0 );
        
        }else{
            $view_data['model_info']->Taxes = $this->db->query($sql)->row();
        }
          //print_r($view_data['model_info']->Taxes);
        $view_data['model_info']->Taxes->spouse = $this->db->query("select * from dependant_and_spouse where relationship='spouse' and client_id=" . $client_id)->row();
        $view_data['model_info']->Taxes->dependants = $this->db->query("select * from dependant_and_spouse where relationship='dependant' and client_id=" . $client_id)->result_object();
        $view_data['model_info']->Taxes->propertys = $this->db->query("select * from dependant_and_spouse where relationship='property' and client_id=" . $client_id)->result_object();
        $view_data['model_info']->Taxes->businesses = $this->db->query("select * from dependant_and_spouse where relationship='business' and client_id=" . $client_id)->result_object();
        
        $view_data['model_info']->user = $this->db->query("select * from clients where id=" . $client_id)->row();
    ...
    private function _make_row($data, $custom_fields) {
        ...
        $optoins = "";
        if ($this->can_edit_projects()) {
            $optoins .= modal_anchor(get_uri("projects/modal_form?client_id=" .$data->client_id), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_project'), "data-post-id" => $data->id));
        }
